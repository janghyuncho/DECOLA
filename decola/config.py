...
def add_detic_config(cfg):
    ...
    _C.DATALOADER.PSEUDO_LABEL_TRAINING_SCORE_THRES = 0.0 
    ...
    _C.MODEL.DETR.TWO_STAGE = True
    _C.MODEL.DETR.WITH_BOX_REFINE = True 
    _C.MODEL.DETR.FOCAL_ALPHA = 0.25
    _C.MODEL.DETR.NHEADS = 8
    _C.MODEL.DETR.DROPOUT = 0.1
    _C.MODEL.DETR.DIM_FEEDFORWARD = 1024
    _C.MODEL.DETR.ENC_LAYERS = 6
    _C.MODEL.DETR.DEC_LAYERS = 6
    _C.MODEL.DETR.PRE_NORM = False
    _C.MODEL.DETR.HIDDEN_DIM = 256
    _C.MODEL.DETR.NUM_OBJECT_QUERIES = 300
    _C.MODEL.DETR.USE_FED_LOSS = True 
    _C.MODEL.DETR.IN_FEATURES = ('res3','res4','res5')
    _C.MODEL.DETR.LOOK_FORWARD_TWICE = True 
    _C.MODEL.DETR.MIXED_SELECTION = False 
    _C.MODEL.DETR.ORACLE_EVALUATION = False 
    _C.MODEL.DETR.TEST_SCORE_THRESHOLD = 0.0 
    _C.MODEL.DETR.USE_ADAPTIVE_NUM_QUERY_FOR_EVALUATION = False 
    _C.MODEL.DETR.PER_CLASS_NUM_QUERY = 0 

    # joint training
    _C.MODEL.DETR.NUM_CLASSES_LIST = []
    _C.MODEL.DETR.USE_FED_LOSS_LIST = []


    # ov-coco
    _C.MODEL.DETR.IGNORE_ZERO_CATS = False 
    _C.MODEL.DETR.CAT_FREQ_PATH = ''

    # weak supervision
    _C.MODEL.DETR.WEAK_WEIGHT = 0.1
    _C.MODEL.DETR.IMAGE_LABEL_LOSS = 'max_size'
    _C.MODEL.DETR.USE_ZEROSHOT_CLS = True  
    _C.MODEL.DETR.ZEROSHOT_WEIGHT_PATH = 'datasets/metadata/lvis_v1_clip_a+cname.npy'
    _C.MODEL.DETR.ZEROSHOT_WEIGHT_DIM = 512
    _C.MODEL.DETR.NORM_WEIGHT = True
    _C.MODEL.DETR.NORM_TEMP = 50.0 
    _C.MODEL.DETR.USE_BIAS = -4.59511985013459 # >= 0: not use

    # eval config 
    _C.TEST = CN()
    _C.TEST.EVAL_PERIOD = 0
    _C.TEST.DETECTIONS_PER_IMAGE = 300
    _C.TEST.DETECTIONS_PER_CLASS_IMAGE = -1
    _C.TEST.JUST_STORE_PREDICTIONS = False 
    _C.TEST.PREDICTION_SAVE_FILENAME = ""
    ...
    _C.WANDB.PROJECT = ""
    _C.WANDB.GROUP = ""
    _C.WANDB.RUN_NAME = "decola"
    _C.WANDB.DISABLE_WANDB = True 
    _C.WANDB.VIS_PERIOD = 0
