...
from ..decola.decola_zero_shot_classifier import DECOLA_ZeroshotClassifier
from .decola_deformable_detr import DECOLA_DETR
from .decola_deta_criterion import Set_DECOLA_Criterion, SetCustomCriterion
from ..decola.decola_matcher import DECOLA_HungarianMatcher
from models.matcher import HungarianMatcher
from ..common import MaskedBackbone
from ..decola.utils import clone_module, detach_module
...
class DECOLA_DETA(nn.Module):
    def __init__(self, cfg):
        ...
        # decola-related 
        prob_a_object = cfg.MODEL.DECOLA.PROB_A_OBJECT
        ...
        self.test_prob_a_object = cfg.MODEL.DECOLA.TEST_PROB_A_OBJECT
        ...
        transformer = DECOLA_DeformableTransformer(
            ...
            assign_first_stage=cfg.MODEL.DECOLA.DETA.ASSIGN_FIRST_STAGE,
        ...
        self.decola_criterion = Set_DECOLA_Criterion(
            ...
            assign_first_stage=cfg.MODEL.DECOLA.DETA.ASSIGN_FIRST_STAGE,
            assign_second_stage=cfg.MODEL.DECOLA.DETA.ASSIGN_SECOND_STAGE,
            num_queries=num_queries,
        )
    ...
    def forward_self_labeling(self, batched_inputs):
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            use_a_object = self.test_prob_a_object == 1.0
        ...
        if self.training:
            ...
            and (self.iter_count % self.vis_period == 0):
                with torch.no_grad():
                    # NOTE: gt_instances contains annotations BEFORE normalization, which is in the original image coord. 
                    # batch_image_sizes = nested_tensor_from_tensor_list(images).tensors.shape[2:]
                    # batch_image_sizes = torch.as_tensor(batch_image_sizes, device=self.device)[None].repeat(len(gt_instances), 1)
                    ...
                    # real_image_sizes = torch.as_tensor([t.image_size for t in gt_instances], device=self.device)
                    # print(batch_image_sizes, image_sizes, flush=True)
            ...
            self.iter_count += 1
            # print(ann_type, self.detr.transformer.use_a_object, loss_dict['loss_image_decola'], flush=True)
    ...
    def nms_post_process(self, outputs, target_sizes):
        ...
        for b in range(bs):
            ...
            assert self.test_topk == 300
            keep_inds = batched_nms(box, score, lbls, 0.7)[:self.test_topk]
            # results.append({
            ...
            r.pred_boxes = Boxes(box[keep_inds])
            r.scores = score[keep_inds]
            r.pred_classes = lbls[keep_inds]
    ...
    def post_process(self, outputs, target_sizes, score_thres=None):
        ...
        """
        if self.cfg.MODEL.DECOLA.DETA.ASSIGN_SECOND_STAGE:
            return self.nms_post_process(outputs, target_sizes)
    ...
    def decola_nms_post_process(self, outputs, target_sizes, score_thres=None):
        out_logits, out_bbox = outputs['pred_logits'], outputs['pred_boxes']
        ...
        all_scores = prob.view(bs, n_queries * n_cls).to(out_logits.device)
        all_indexes = torch.arange(n_queries * n_cls)[None].expand(bs, n_queries * n_cls).to(out_logits.device)
        all_boxes = torch.div(all_indexes, out_logits.shape[2], rounding_mode="floor")
    ...
    def prepare_targets(self, targets, batched_inputs):
        ...
        for targets_per_image, x in zip(targets, batched_inputs):
            ...
            new_targets.append({"labels": gt_classes, "boxes": gt_boxes, 'file_name': x['file_name']})
            if 'ann_type' in x:
    ...
    def prepare_offline_pseudo_labels(self, output, targets, image_sizes, batched_inputs):
        # assert len(output['pred_logits']) == len(target_sizes)
        # assert target_sizes.shape[1] == 2
        # assert len(targets) == 1, "batch size must be 1."
