...
class DECOLA_DeformableTransformer(DeformableTransformer):
    ...
    def generate_proposals(self, output_scores, output_coords, output_memory, enc_prompt_inds, enc_prompt_embeds):
        if self.use_a_object:
            return self.generate_object_prompt_proposals(output_scores, output_coords, output_memory, enc_prompt_inds, enc_prompt_embeds)
        else:
            return self.generate_prompt_specific_proposals(output_scores, output_coords, output_memory, enc_prompt_inds, enc_prompt_embeds)

    def generate_object_prompt_proposals(self, output_scores, output_coords, output_memory, prompt_inds, enc_prompt_embeds):
        """
        output_scores: (B,S,1)
        output_coords: (B,S,4)
        output_memory: (B,S,C)
        ...
        topk_proposals = torch.topk(output_scores[..., 0], topk, dim=1)[1]
        ...
        return topk_coords_unact, None, topk_feats, prompt_embeds
    ...
    def generate_prompt_specific_proposals(
        ...
        topk_scores = per_prompt_topk_scores.flatten(1, 2) # B,NK
        ...
        return topk_coords, prompt_inds, topk_feats, prompt_embeds
        

    def forward_decoder(self, decoder_input):
        memory, spatial_shapes, level_start_index, valid_ratios, mask_flatten, output_memory, output_proposals, targets = decoder_input
        ...
        topk_coords_unact, topk_prompt_inds, topk_feats, topk_prompt_embeds = \
            self.generate_proposals(enc_outputs_class, enc_outputs_coord_unact, output_memory, enc_prompt_inds, enc_prompt_embeds)
        ...
        return hs, init_reference_out, inter_references_out, topk_prompt_inds, enc_outputs_class, enc_outputs_coord_unact, enc_prompt_inds
