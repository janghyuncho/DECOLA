...
class CustomLVISEval(LVISEval):
    def __init__(self, lvis_gt, lvis_dt, iou_type="segm", category_ids=[]):
        """Constructor for CustomLVISEval.
        Args:
            ...
            (vincent) category_ids: if provided, only evaluate on those classes.
        """
        ...
        if len(self.category_ids) > 0:
            self.params.cat_ids = sorted(self.category_ids)
